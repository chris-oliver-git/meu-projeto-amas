...
import familiaHome from './familia-home.png';
import { AmasRedePage, AmasBeneficiosPage } from './Parceiros.jsx'; // <-- Importa os novos componentes estruturados
...
export default function App() {
  ...
  // Função de roteamento adaptada para ler /rede e /beneficios
  ...
  // Novo menu estruturado conforme solicitado
  const LINKS_NAVEGACAO = [
    { id: 'inicio', label: 'Início' },
    { id: 'planos', label: 'Planos' }, 
    { id: 'rede', label: 'Rede' },       // Substituiu Parceiros
    { id: 'beneficios', label: 'Benefícios' }, // Nova aba exclusiva
        ...
        {paginaAtiva === 'inicio' ? (
          <>
            ...
            <section className="relative bg-[#0b1f52] overflow-hidden min-h-[500px] sm:min-h-[580px] flex items-center justify-center px-4 sm:px-6">
              ...
              <div className="relative z-10 max-w-4xl mx-auto text-center py-16 space-y-6">
                <h1 className="text-3xl sm:text-5xl lg:text-[54px] font-black text-white tracking-tight leading-[1.15]">Cuidando da Sua Saúde com Carinho e Economia.</h1>
                <p className="text-base sm:text-xl text-gray-200 font-medium max-w-2xl mx-auto">Acesso facilitado a uma ampla rede de saúde sem burocracia. Para você e toda sua família.</p>
                <div className="pt-4"><button onClick={() => navegarPara('planos')} className="bg-[#ff6b00] hover:bg-[#e05e00] text-white font-black px-10 py-4 rounded-xl text-md sm:text-lg">Conheça Nossos Planos</button></div>
              </div>
            </section>
          ...
          <AmasRedePage setPaginaAtiva={navegarPara} /> // Renderiza apenas as clínicas
        ) : paginaAtiva === 'beneficios' ? (
          <AmasBeneficiosPage setPaginaAtiva={navegarPara} /> // Renderiza apenas os descontos comerciais
        ) : paginaAtiva === 'anexos' ? (
      ...
      <footer className="bg-[#0b1f52] text-[#a5b4fc] pt-12 pb-8 border-t border-white/5 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 text-center sm:text-left">
          ...
          <div className="space-y-3">
            ...
            <div className="text-[14px] text-[#a5b4fc]/80"><p className="text-white font-semibold">AMAS MULTI SAÚDE LTDA</p><p>CNPJ 31.798.999/0001-90</p></div>
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 mt-8 border-t border-white/10 text-center text-xs text-[#a5b4fc]/60">
          <p>Copyright © 2026 AMAS MULTI SAUDE LTDA. Todos os direitos reservados. Desenvolvido por <span className="text-[#ff6b00] font-bold">Íon Digital</span></p>
        </div>
      </footer>
